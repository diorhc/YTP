...
(function () {
  ...
  const LANGUAGE_NAMES = {
    en: 'English', ru: '???????', kr: '???', fr: 'Français', du: 'Nederlands', cn: '????', tw: '????', jp: '???', tr: 'Türkçe', es: 'Español', pt: 'Português', de: 'Deutsch', it: 'Italiano', pl: 'Polski', uk: '??????????', sv: 'Svenska', no: 'Norsk', da: 'Dansk', fi: 'Suomi', cs: 'Ceština', sk: 'Slovencina', hu: 'Magyar', ro: 'Româna', bg: '?????????', hr: 'Hrvatski', sr: '??????', sl: 'Slovenšcina', el: '????????', lt: 'Lietuviu', lv: 'Latviešu', et: 'Eesti', mk: '??????????', sq: 'Shqip', bs: 'Bosanski', is: 'Íslenska', ca: 'Català', eu: 'Euskara', gl: 'Galego', ar: '???????', he: '?????', fa: '?????', sw: 'Kiswahili', zu: 'isiZulu', af: 'Afrikaans', am: '????', hi: '??????', th: '???', vi: 'Ti?ng Vi?t', id: 'Bahasa Indonesia', ms: 'Bahasa Melayu', bn: '?????', ta: '?????', te: '??????', mr: '?????', gu: '???????', kn: '?????', ml: '??????', pa: '??????', fil: 'Filipino', km: '?????????', lo: '???', my: '??????', ne: '??????', si: '?????', az: 'Az?rbaycanca', be: '??????????', hy: '???????', ka: '???????', kk: '?????', ky: '????????', mn: '??????', tg: '??????', uz: 'O?zbekcha',
  };
