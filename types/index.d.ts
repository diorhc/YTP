...
/**
 * DOM Manager interface exposed by `YouTubePlusDOMManager`.
 * Includes a small, well-tested subset used across the project.
 */
interface YouTubePlusDOMManager {
  ...
  /** Query selector with optional cache bypass used by DOMManager */
  querySelector: (selector: string, nocache?: boolean) => HTMLElement | null;
  clearCache: () => void;
  waitForElement: (
    selector: string,
    timeout?: number,
...
/** Attributes object used by DOMManager#createElement */
type CreateElementAttrs = {
  id?: string;
  className?: string;
  dataset?: Record<string, string>;
  style?: Partial<CSSStyleDeclaration> | Record<string, string>;
  // event handlers may be provided as onClick/onChange etc.
  [key: string]: any;
};

/**
 * Notification manager minimal interface
 */
interface YouTubePlusNotificationManager {
  show: (message: string, options?: NotificationOptions) => HTMLElement | null;
  remove: (notification: HTMLElement | null) => void;
  clearAll: () => void;
  success: (message: string, options?: NotificationOptions) => HTMLElement | null;
  error: (message: string, options?: NotificationOptions) => HTMLElement | null;
  info: (message: string, options?: NotificationOptions) => HTMLElement | null;
  activeNotifications: Set<HTMLElement>;
}
...
type NotificationOptions = {
  duration?: number;
  position?: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left' | string | null;
...
declare global {
  interface Window {
    ...
    YouTubePlusDOMManager?: YouTubePlusDOMManager;
    YouTubePlusNotificationManager?: YouTubePlusNotificationManager;
